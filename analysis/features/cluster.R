...
pathresults <- here::here("results", "features_real")
...
cluster.lab.plot <- function(n_im, clust, lg_sz, ncol, FISH) {
  ...
  df_rgb <- na.omit(as.data.frame(im_no_NA, wide = "c")) %>%
    dplyr::mutate(rgb.val = rgb(c.1, c.2, c.3))

  set.seed(1) #important to obtain always the same clusters !
  kmeans <- kmeans(
    na.omit(df[, c("c.1", "c.2", "c.3")]),
    centers = clust,
    iter.max = 100,
    nstart = 1
  )
  ...
  p[[2]] <- ggplot(df, aes(x, y)) +
    geom_raster(aes(fill = kColours)) +
    scale_fill_identity() +
    theme(
    ...
    xlab("") +
    ylab("") +
    ggtitle(paste("nb_clust= ", clust)) +
    scale_y_reverse()

  if (min(kmeans$centers[, "c.2"]) > 10) minx <- 5 else
    minx <- min(kmeans$centers[, "c.2"])
  if (min(kmeans$centers[, "c.3"]) > 10) miny <- 5 else
    miny <- min(kmeans$centers[, "c.3"])
  ...
  bg <- ggplot(bkg, aes(x = c.2, y = c.3)) +
    geom_point(size = 1, shape = 15, colour = bkg$col) +
  ...
  g <- lapply(4:(clust + 3), function(i) {
    ...
    ggplot(df, aes(x, y)) +
      geom_raster(aes(fill = kColours_cl)) +
      scale_fill_identity() +
      xlab("") +
      ylab("") +
      ggtitle(name) +
      ...
      theme(
        ...
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        plot.title = element_text(size = lg_sz)
      )
  })

  if (FISH == TRUE) (do.call(gridExtra::grid.arrange, c(p, g))) else
    do.call(gridExtra::grid.arrange, c(g))
}
...
cluster_stat <- function(cluster_raw, n_im, clust, colors_data) {
  ...
  shape_id <- c(
    ...
    "SH_perimeter_mean",
    "SH_perimeter_sd",
    "SH_perim.area.ratio_mean",
    "SH_perim.area.ratio_sd",
    "SH_core.area.index_mean",
    "SH_core.area.index_sd"
  )
  ...
  varout <- cbind.data.frame(
    do.call(cbind, lapply(cie_id, function(i) get(i))),
...
ggplot2::ggsave(
  filename = here::here("outputs", "Cluster_color.jpg"),
  plot = plot,
  width = 18,
  height = 24,
  units = "cm",
...
#to based on previous studies (used in Tribot et al. 2018. Confronting
#species aesthetics with ecological functions of coral reef fishes.
#Scientific Reports, 8, 11733.).

pict <- length(files)

raw_data <- as.data.frame(do.call(
  rbind,
  parallel::mclapply(
    1:pict,
    function(i) cluster(n_im = i, clust = 9),
    mc.cores = parallel::detectCores()
  )
))
stat_data <- as.data.frame(do.call(
  rbind,
  parallel::mclapply(
    1:pict,
    function(i) cluster_stat(cluster_raw = raw_data, n_im = i, clust = 9),
    mc.cores = parallel::detectCores()
  )
))

stat_data$name <- gsub("_front_1", "", stat_data$name)
stat_data$name <- gsub("_front_2", "", stat_data$name)
stat_data$name <- gsub("_face_1", "", stat_data$name)
stat_data$name <- gsub("_face_2", "", stat_data$name)

stat_data <- do.call(
  rbind,
  lapply(unique(stat_data$name), function(name) {
    #name <- "Ailuropoda_melanoleuca"
    rowIndex <- which(stat_data$name == name)
    data.frame(Image_id = name, t(colMeans(stat_data[rowIndex, -1])))
  })
)

write.csv2(
  stat_data,
  here::here("results", "features", "cluster.csv"),
  row.names = F
)
